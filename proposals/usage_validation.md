...
Since it must be possible to override all restrictions from user code, all validation rules must be public, and unless we wanna override all the checks, we need to be able to refer to those checks by name. Users of library code might want to add validation to protect their own code from stupid mistakes, so we need the ability to add additional checks to imported identifiers, outside the declarations of structs and outside the modules where things are defined, aka user code.
